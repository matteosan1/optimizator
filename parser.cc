...
void getVar(int index, vector<string> values,
	    std::vector<float>& retF, std::vector<string>& retS) {
  
  vector<string>::const_iterator it;
  vector<string> stringValues;  
  for (int i=0; i<5; i++) {
    it = values.begin()+index+i;
    if (i==0 or i==1)
      retS.push_back(*it);
    
    if (i==2 or i==3)     
      retF.push_back(boost::lexical_cast<float>(*it));
